...
def gradientDescent(p):
    ...
    # 초기값에 대한 함수값을 계산
    valueC = evaluate(current, p)
    # 계산을 반복하며 더 나은 solution을 탐색
    while True:
        # 다음에 Gradient를 따라 이동할 위치 판단
...
def takeStep(x, v, alpha, dx, p):
    ...
    for i in range(len(grad)):
        # alpha값과 grad를 이용하여 업데이트
        # numpy 배열인 경우 for문 밖에서 편하게 할 수 있으나 list는 for문 필요
        x_new[i] -= alpha*grad[i]

        # 업데이트 된 x_new 값이 domain(low, up) 범이 안에 있는지 확인한 후
        # 범위를 벗어난 경우 x 업데이트 하지않고 return
...
def gradient(x, v, dx, p):
    ...
    for i in range(len(x)):
        #  반복할 때마다 초기화
        # x_new => x[i]에 dx만큼 더한 x, x_new는 slicing을 이용해 복사해서 만들자
        x_new = x[:]
        x_new[i] += dx
        # v_new = evaluate를 이용해서 계산
